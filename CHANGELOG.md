...
- Improved the fzf popup to use configurable status/icon/label/title rows, per-agent icon options, a 20% list and 80% preview split, ANSI-preserving preview, joined tmux physical wraps, no fzf wrap markers, bottom-following preview with shortcut footer, enter-only jump, escape close, extra fzf theme options, and refresh on demand.
